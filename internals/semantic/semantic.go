...
type Semantic struct {
	...
	currentAnonymousFunctionBeingAnalyzed *ast.AnonymousFunction
	returns                               bool
	currentExpectedReturnType             ctypes.Type
	Errors                                []error
	Warnings                              []error
	insideBreakableBlock                  bool
	expectConstantExpression              bool
	expectNonConstantExpression           bool
...
func (s *Semantic) errorWithStatement(msg string, tok token.Token) {
	msg += "\n" + s.getCurrentStatementLineFormatted()
	s.error(msg, tok)
}

func (s *Semantic) warningWithStatement(msg string, tok token.Token) {
	msg += "\n" + s.getCurrentStatementLineFormatted()
	s.warning(msg, tok)
}
...
func (s *Semantic) analyzeStatement(statement ast.Statement) {
	...
	case *ast.BreakStatement:
		if !s.insideBreakableBlock {
			s.errorWithStatement("Unexpected break statement", statementType.Token)
		}
		return
	...
	case *ast.ContinueStatement:
		if !s.insideBreakableBlock {
			s.errorWithStatement("Unexpected continue statement", statementType.Token)
		}
		return
	}
...
func (s *Semantic) analyzeAssigmentStatement(assign *ast.AssignmentStatement) {
	...
	s.expectNonConstantExpression = false
	if !s.areTypesEqualIncludingUnions(left, right) {
...
func (s *Semantic) analyzeAnonymousFunction(anonymousFunction *ast.AnonymousFunction) ctypes.Type {
	temporaryReturns := s.returns
	s.returns = false
	tmpAnon := s.currentAnonymousFunctionBeingAnalyzed
	tmpFun := s.currentFunctionBeingAnalyzed
	s.currentFunctionBeingAnalyzed = anonymousFunction.FunctionType
	s.currentAnonymousFunctionBeingAnalyzed = anonymousFunction
	s.analyzeFunctionType(anonymousFunction.Token, anonymousFunction.FunctionType, anonymousFunction.Block)
	s.currentFunctionBeingAnalyzed = tmpFun
	s.currentAnonymousFunctionBeingAnalyzed = tmpAnon
	s.returns = temporaryReturns
...
func (s *Semantic) analyzeUnionStatement(statementType *ast.UnionStatement) {
	// nit: literally the same function as analzyzeStructStatement, rewrite it to a single one.
	s.definedTypes[statementType.Type.Name] = statementType.Type
	for i, t := range statementType.Type.Fields {
		t = s.UnwrapAnonymous(t)
		// Get underlying type
...
func (s *Semantic) analyzeBuiltinCall(call *ast.BuiltinCall) ctypes.Type {
	...
	s.errorWithStatement("unknown builtin call", call.Token)
	return ctypes.TODO()
}
...
func (s *Semantic) UnwrapAnonymous(t ctypes.Type) ctypes.Type {
	...
	if anonymous, ok := t.(*ctypes.Anonymous); ok {
		...
		if !ok {
			...
			s.errorWithStatement("Couldn't guess type "+ast.RetrieveID(anonymous.Name)+", maybe spelt the type wrong? These are the defined types in the module"+" "+ast.RetrieveID(module)+":\n"+typesDefined, s.currentStatementBeingAnalyzed.GetToken())
			return ctypes.TODO()
		}
...
func (s *Semantic) swapTypes(t ctypes.Type, toSwap ctypes.Type) ctypes.Type {
	...
	if toSwap == ctypes.TODO() {
		...
		if _, ok := trueType.(*ctypes.Anonymous); (ok && trueType == t) || trueType == nil {
			s.errorWithStatement("unknown type "+t.String(), s.currentStatementBeingAnalyzed.GetToken())
		}
		return trueType
	}
...
func (s *Semantic) areTypesEqual(first, second ctypes.Type) bool {
	...
	if ctypes.IsUnion(first) || ctypes.IsUnion(second) {
		s.warningWithStatement("we can't coerce into a union here,\ntry putting this statement on its own declaration or assignment to fix this warning", s.currentStatementBeingAnalyzed.GetToken())
	}

	return false
}

// areTypesEqualIncludingUnions repetition of areTypesEqual but with union checks
func (s *Semantic) areTypesEqualIncludingUnions(first, second ctypes.Type) bool {
	first = s.UnwrapAnonymous(first)
	second = s.UnwrapAnonymous(second)
	...
	if ctypes.IsPointer(first) && ctypes.IsPointer(second) {
		return s.areTypesEqualIncludingUnions(first.(*ctypes.Pointer).Inner, second.(*ctypes.Pointer).Inner)
	}
	...
	if firstFunc, ok := first.(*ctypes.Function); ok {
		...
		for i, param := range firstFunc.Parameters {
			if !s.areTypesEqualIncludingUnions(secondFunc.Parameters[i], param) {
				return false
			}
		}
		return s.areTypesEqualIncludingUnions(firstFunc.Return, secondFunc.Return)
	}
	...
	if ctypes.IsArray(first) {
		...
		return fArray.Length == sArray.Length && s.areTypesEqualIncludingUnions(fArray.Inner, sArray.Inner)
	}
...
func (s *Semantic) analyzeIndexAccess(indexAccess *ast.IndexAccess) ctypes.Type {
	...
	if !ctypes.IsArray(leftType) && !ctypes.IsPointer(leftType) {
		s.errorWithStatement("expected a pointer or an array for an index access, instead we got "+leftType.String(), indexAccess.Token)
	}
	...
	s.errorWithStatement("mismatched types on index access, internal compiler bug", indexAccess.Token)
...
func (s *Semantic) retrieveModule(moduleName string) *Semantic {
	...
	if !ok {
		s.errorWithStatement("undefined module "+ast.RetrieveID(moduleName), s.currentStatementBeingAnalyzed.GetToken())
		return s
	}
...
func (s *Semantic) analyzeStructLiteral(structLiteral *ast.StructLiteral) ctypes.Type {
	...
	if err != nil {
		s.errorWithStatement(err.Error(), structLiteral.Token)
	...
	if !ok {
		s.errorWithStatement("undefined struct "+ast.RetrieveID(structLiteral.Name)+": "+structLiteral.String(), structLiteral.Token)
		return ctypes.TODO()
	}
	...
	for _, value := range structLiteral.Values {
		...
		if !ok {
			s.errorWithStatement("undefined attribute on struct literal "+value.Name, structLiteral.Token)
		}
		expression := s.analyzeExpression(value.Expression)
		if !s.areTypesEqualIncludingUnions(structType.Fields[index], expression) {
...
func (s *Semantic) analyzeArrayLiteral(arrayLiteral *ast.ArrayLiteral) ctypes.Type {
	...
	if int(arrayType.Length) < len(arrayLiteral.Values) {
		s.errorWithStatement("expected an array of length "+strconv.FormatInt(arrayType.Length, 10)+" or less", arrayLiteral.Token)
	}
...
func (s *Semantic) analyzeFunctionCall(call *ast.Call) ctypes.Type {
	s.throwInvalidOperationForConstant("can't call non constant functions for a constant expression", call)
	...
	if funcType, ok = possibleFuncType.(*ctypes.Function); !ok {
		s.errorWithStatement("can't call non function "+call.Left.String()+" of type "+possibleFuncType.String(), call.Token)
		return ctypes.TODO()
	}
	...
	if len(call.Parameters) != len(funcType.Parameters) && !funcType.InfiniteParameters {
		...
		s.errorWithStatement(fmt.Sprintf("mismatch number of parameters, expected %d, got %d", len(funcType.Parameters), len(call.Parameters)), call.Token)
	}
...
func (s *Semantic) analyzeIdentifier(identifier *ast.Identifier) ctypes.Type {
	...
	if identifierType := s.variables.Get(identifier.Name); identifierType != nil {
		...
		if identifierType.parentFunction != s.currentFunctionBeingAnalyzed && identifierType.parentFunction != nil {
			s.currentAnonymousFunctionBeingAnalyzed.CapturedVariables =
				append(s.currentAnonymousFunctionBeingAnalyzed.CapturedVariables, identifier.Name)
			identifierTypeUnwrap := s.UnwrapAnonymous(s.replaceAnonymous(identifierType.Type))
			s.variables.Add(identifier.Name, &SemanticType{Type: identifierTypeUnwrap, parentFunction: s.currentFunctionBeingAnalyzed})
			return identifierTypeUnwrap
		}

		if !identifierType.IsConstant && s.expectConstantExpression {
			s.errorWithStatement("this expression should be constant and '"+identifier.String()+"' is not a constant variable", identifier.Token)
		}

		if identifierType.IsConstant && s.expectNonConstantExpression {
			s.errorWithStatement("you are trying to mutate a constant, '"+identifier.String()+"' is a constant variable", identifier.Token)
		}

		return identifierType.Type
	}

	s.errorWithStatement("undefined variable "+ast.RetrieveID(identifier.Name), identifier.Token)
	return ctypes.TODO()
}
...
func (s *Semantic) analyzePrefixOperation(prefixOperation *ast.PrefixOperation) ctypes.Type {
	...
	if prefixOperation.Operation == ops.AddOne || prefixOperation.Operation == ops.SubtractOne {
		s.throwInvalidOperationForConstant("you can't mutate on constant expressions", prefixOperation)
		if !ctypes.IsNumeric(t) {
			s.typeMismatchError(prefixOperation.String(), prefixOperation.Right, prefixOperation.Token, ctypes.LiteralToType("i32"), t)
			return t
		}
		...
		if _, isInteger := prefixOperation.Right.(*ast.Integer); !isInteger {
			if _, isFloat := prefixOperation.Right.(*ast.Float); isFloat {
				s.errorWithStatement("you can't add a number literal", prefixOperation.Token)
			}
		} else {
			s.errorWithStatement("you can't add a number literal", prefixOperation.Token)
		}
	...
	if prefixOperation.Operation == ops.BinaryAND {
		s.throwInvalidOperationForConstant("you can't ref on constant expressions", prefixOperation)
		prefixOperation.Type = &ctypes.Pointer{Inner: t}
		return prefixOperation.Type
	}

	if prefixOperation.Operation == ops.Multiply {
		s.throwInvalidOperationForConstant("you can't deref on constant expressions", prefixOperation)
		if ptr, ok := t.(*ctypes.Pointer); !ok {
	...
	s.errorWithStatement("unknown prefix operator to analyze", prefixOperation.Token)
...
func (s *Semantic) analyzeModuleAccess(module *Semantic, binaryOp *ast.BinaryOperation) ctypes.Type {
	...
	if !ok {
		s.errorWithStatement("expected identifier for module access, got "+binaryOp.Right.String(), binaryOp.Token)
		return ctypes.TODO()
	}
	...
	if accessedElement == nil {
		s.errorWithStatement(ast.RetrieveID(identifier.Name)+" does not exist in the specified module", binaryOp.Token)
		return ctypes.TODO()
	}
...
func (s *Semantic) analyzeFieldAccess(binaryOperation *ast.BinaryOperation) ctypes.Type {
	...
	if ptr, isPointer := left.(*ctypes.Pointer); isPointer {
		s.throwInvalidOperationForConstant("you can't access pointer fields", binaryOperation)
...
func (s *Semantic) analyzeImport(importStatement *ast.ImportStatement) {
	...
	tree := p.Parse()
	if len(p.Errors) > 0 {
		s.errorWithStatement("error parsing file imported on path "+importStatement.Path.String(), importStatement.Token)
...
// Returns the same type but with the anonymous type replaced.
